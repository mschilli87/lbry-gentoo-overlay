...
SRC_URI="https://github.com/google/${PN}/archive/v${My_PV}.tar.gz -> ${PN}-${My_PV}.tar.gz
	https://github.com/google/protobuf/pull/235.patch -> ${PN}-2.6.1-protoc-cmdline.patch"
...
src_prepare() {
	...
	[[ ${CHOST} != *-darwin* ]] && epatch "${FILESDIR}"/${PN}-3.0.0_beta2-asneeded-2.patch

	# fix build with emacs-24.4 (bug #524100)
	epatch "${FILESDIR}"/${PN}-2.5.0-emacs-24.4.patch
	epatch "${DISTDIR}"/${PN}-2.6.1-protoc-cmdline.patch
